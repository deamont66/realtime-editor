...
Jedná se o nejčastěji používaný algoritmus pro kolaborativní spolupráci ve skutečném čase.
...
Operace označuje změnu v dokumentu a u čistě textových dokumentech rozlišujeme tři druhy operací:
\begin{itemize}
    ...
    \item přidej řetězec s parametrem který označuje řetězec k přidání.
\end{itemize}
...
Mějme dokument A, soubory operací z A do B, z A do C, pak aplikací změn z druhého souboru na dokument B získáme nový soubor (soubor transformovaný).
...
\begin{enumerate}
    ...
    \item klient po přijetí souboru může odeslat nastřádaný soubor zkombinovaných operací (pokračuje bodem~\ref{krok:op_zmena}) nebo čeká na další změnu a pokračuje znovu bodem~\ref{krok:op_kombinace}.
\end{enumerate}
...
Tento krok snižuje náročnost celého algoritmu omezením počtu současných operací, které by musel server kombinovat.
...
Implementace obecného algoritmu \gls{OT} pro různé typy strukturovaných souborů není jednoduchá a potvrzuje to i následující tvrzení tehdejšího vývojáře Google Wave Josepha Gentle:

\enquote{Unfortunately, implementing OT sucks.
There's a million algorithms with different tradeoffs, mostly trapped in academic papers.
The algorithms are really hard and time consuming to implement correctly.}~\cite{vyvojar} přeložil Jiří Šimeček.

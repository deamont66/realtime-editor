...
Wave Federation protokol byl navržen jako rozšíření algoritmu \gls{OP} (více o algoritmu v~\ref{subsec:operationalTransformation}) společností Google a zasloužil se o důležité vylepšení v podobě potvrzování každé přijaté operace serverem.~\cite{wave:apache, wave:google}
...
Google Docs využívá Google Apps API, které je také postaveno nad algoritmem \gls{OP} (více o algoritmu v~\ref{subsec:operationalTransformation})~\cite{docs:conflict} a to včetně vylepšení se kterým přišel Google při vývoji Google Wave~\cite{docs:appsConf}.
...
Knihovna EasySync (a tím tedy i Etherpad samotný) využívá principy algoritmu \gls{OP} (více o algoritmu v~\ref{subsec:operationalTransformation}) a vylepšení ve formě včetně čekání klienta po odeslání operace na potvrzení od serveru~\cite{etherpad:easySync}.
